## Identify stems to remove based on growth rate outliers 
## 11-9-2020
...
# called this "clean_dat" instead of "growdata" 

#---------------------------------------------------------------------------------------------#
# Update DFstatus
#---------------------------------------------------------------------------------------------#
clean_dat$status <- gsub("0", "y", clean_dat$status)
clean_dat$status <- gsub("dead", "y", clean_dat$status)
clean_dat$status<- gsub("broken below", "yy", clean_dat$status)
clean_dat$status <- gsub("^b.*", "yy", clean_dat$status)
clean_dat$status <- gsub("missing", "yyy", clean_dat$status)

clean_dat$status<- gsub("[^y]+", "A", clean_dat$status)

clean_dat$status <- gsub("yyy","missing", clean_dat$status)
clean_dat$status<- gsub("yy", "B", clean_dat$status)
clean_dat$status <- gsub("y", "D", clean_dat$status)

table(clean_dat$status)
table(data$DFstatus)

SPKA <- subset(data, site == "SPKA"); table(SPKA$DFstatus)
SPKS <- subset(data, site == "SPKS"); table(SPKS$DFstatus)
SPKH <- subset(data, site == "SPKH"); table(SPKH$DFstatus)
SPKA <- subset(clean_dat, site == "SPKA"); table(SPKA$status)
SPKS <- subset(clean_dat, site == "SPKS"); table(SPKS$status)
SPKH <- subset(clean_dat, site == "SPKH"); table(SPKH$status)
#---------------------------------------------------------------------------------------------#

#---------------------------------------------------------------------------------------------#
clean_dat$hom <- rep(130, length(growdata$dbh));

# filter to only include alive trees for calculating growth rates
growdata <- filter(clean_dat, status == "A") # EO EDIT missing dataframe in filter function, added
#---------------------------------------------------------------------------------------------#
...
#---------------------------------------------------------------------------------------------#
#p99 <- quantile(SPKS08_2001_2009$annual_increment, prob=0.99); p99
...
#---------------------------------------------------------------------------------------------#
#p99 <- quantile(SPKS08_2009_2014$annual_increment, prob=0.99); p99
# set aside stems to remove: 
...
#---------------------------------------------------------------------------------------------#
# SPKS08 POOLED growth rates (all censuses) 
#---------------------------------------------------------------------------------------------#
...
SPKS801$pool_stem_ID    <- paste0(SPKS801$stemID, "_1") 
...
growthr <- rbind(SPKS08_2001_2009, SPKS08_2009_2014, DNM1_2006_2013, DNM1_2013_2016,
                 ...
                 DNM50_2011_2019, SPKA9_2001_2009, SPKA9_2009_2014, SPKA10_2001_2009, 
                 SPKA10_2009_2014, SPKH4_2001_2008, SPKH4_2008_2014, SPKH5_2001_2008, 
