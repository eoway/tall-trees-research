...
heightdat <- raster("Forest_height_2019_SASIA_subset.tif")

summary(heightdat)
plot(heightdat)

#Elsa Help
NAvalue(heightdat) <- 0 # this function will only accept 1 value, which is why you can't give it 0, 101, 102, and 103
summary(heightdat)

table(heightdat)

plot(heightdat)
# consider further subsetting for analysis to further south: Peninsular Malaysia as northernmost boundary

#save a new version of the raster


#water data
# for the water data, you'll want to create a 'distance to water' raster layer that aligns with the height data
...
#soil data
#EO to NH: see email about soils data
library(XML)
library(rgdal)
library(gdalUtils)
